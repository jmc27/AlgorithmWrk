...
public class MyFileSystem implements FileSystem {
    ...
    FreeMap    freeMap;
    IndirectBlock indirectOne = new IndirectBlock();
    IndirectBlock indirectTwo = new IndirectBlock();
    IndirectBlock indirectThree = new IndirectBlock();
    ...
    public void constructIndirects(){
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {
        ...
        int IBsize = IndirectBlock.COUNT;
        // Here the blockNum is a logical block number refering to a
        // pointer in the inode.
        if(mode == MODE.w) {
        	if(blockNum <= 9){
        		boolean fresh = inode.ptr[blockNum] == 0;
        		if(fresh){
        			if((inode.ptr[blockNum] = freeMap.find()) == 0) 
        				return null;
        		}
        		return new DirectBlock(disk, inode.ptr[blockNum], blockOff, fresh);
        	}
        	else if(blockNum < 10 + IBsize){
        		if(inode.ptr[10] == 0)
                	if((inode.ptr[10] = freeMap.find()) == 0) 
                		return null;
        		if((indirectOne.ptr[blockNum-10] = freeMap.find()) == 0) 
        			return null; 	// find freeblock for datablock;     	
            	disk.write(inode.ptr[10], indirectOne);
            	return new DirectBlock(disk, indirectOne.ptr[blockNum-10], blockOff, true);
        	}
        	else if(blockNum < 10 + IBsize + IBsize * IBsize) {
        		int onesize = (blockNum - 10 - IBsize) / IBsize;
            	int twosize = (blockNum - 10 - IBsize) % IBsize;
            	
            	if(inode.ptr[11] == 0)
            		if((inode.ptr[11] = freeMap.find()) == 0)
            				return null;
            	if((indirectOne.ptr[onesize]) == 0)
            		if((indirectOne.ptr[onesize] = freeMap.find()) == 0)
            			return null;
            	if((indirectTwo.ptr[twosize] = freeMap.find()) == 0)
            		return null;
            	disk.write(inode.ptr[11], indirectOne);
            	disk.write(indirectOne.ptr[onesize], indirectTwo);
            	return new DirectBlock(disk, indirectTwo.ptr[twosize], blockOff, true);
        	}
        	else if(blockNum < 10 + IBsize + IBsize * IBsize + IBsize * IBsize * IBsize) {
        		int onesize = (blockNum-10-IBsize-IBsize*IBsize)/(IBsize*IBsize);
            	int twosize = ((blockNum-10-IBsize-IBsize*IBsize)/IBsize)%IBsize; 
            	int threesize = (blockNum-10-IBsize-IBsize*IBsize)%IBsize;
            	...
            	if(indirectOne.ptr[onesize] == 0)
            		if((indirectOne.ptr[onesize] = freeMap.find()) == 0)
            			return null;
            	if(indirectTwo.ptr[twosize] == 0)
            		if((indirectTwo.ptr[twosize] = freeMap.find()) == 0)
            			return null;
            	if((indirectThree.ptr[threesize] = freeMap.find()) == 0)
            		return null;
            	disk.write(inode.ptr[12], indirectOne);
            	disk.write(indirectOne.ptr[onesize], indirectTwo);
            	disk.write(indirectTwo.ptr[twosize], indirectThree);
            	return new DirectBlock(disk, indirectThree.ptr[threesize], blockOff, true);
        	}
        	return null;
        }
        else{
        	if(blockNum <= 9) {
        		...
        		if(inode.ptr[10] == 0) 
        			 System.out.println("NO SINGLE INDIRECTION");
         	   disk.read(inode.ptr[10], indirectOne);       	
         	   return new DirectBlock(disk, indirectOne.ptr[blockNum-10], blockOff, false);
        	}
        	else if(blockNum < 10 + IBsize + IBsize * IBsize) {
        		if(inode.ptr[11] == 0) 
        			System.out.println("NO DOUBLE INDIRECTION");
        		disk.read(inode.ptr[11], indirectOne); 
        		disk.read(indirectOne.ptr[(blockNum - 10 - IBsize) / IBsize], indirectTwo);      	
        		return new DirectBlock(disk, indirectTwo.ptr[(blockNum - 10 - IBsize) % IBsize], blockOff, false);
        	}
        	else if(blockNum < 10 + IBsize + IBsize * IBsize + IBsize * IBsize * IBsize) {
        		disk.read(inode.ptr[12], indirectOne); 
         	   disk.read(indirectOne.ptr[(blockNum - 10 - IBsize - IBsize * IBsize) / (IBsize * IBsize)], indirectTwo);  
         	   disk.read(indirectTwo.ptr[((blockNum - 10 - IBsize - IBsize * IBsize) / IBsize) % IBsize], indirectThree);  
         	   return new DirectBlock(disk, indirectThree.ptr[(blockNum - 10 - IBsize - IBsize * IBsize) % IBsize], blockOff, false);
        	}
        	return null;
        }
